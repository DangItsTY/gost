function eyeballer(x, y, z) {
	...
	this.act = function() {
		...
		if (theHelper.isInRange(this, THEPLAYER, 2) && this.actionState != 2) {
			this.puff();
			this.stopTimer = 0;
			this.stopTimerMax = 2;
			this.actionState = 2;
	...
	this.puff = function() {
		theObjectFactory.create(new spit(this.x, this.y + UP_DIRECTION, theObjectFactory.zindex.projectiles));
		theObjectFactory.justAdded().speed = 0;
		theObjectFactory.justAdded().sprite.scale.set(1, 1, 1);
		theObjectFactory.justAdded().sprite.material.rotation = Math.PI/2;
	};
